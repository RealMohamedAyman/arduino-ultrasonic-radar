...
void setup() {
  ...
  // Since we have sent data from arduino serial in the form (angle,distance.) then we will need as processing here to read until every dot (.)
  myPort.bufferUntil('.'); // reads the data from the serial port up to the character '.'. So actually it reads this: angle,distance.
}

void draw() {
  // Filling screen with RGB Colors
  fill(98, 245, 31);
